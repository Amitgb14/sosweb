    ...
    for files in os.listdir(report_path):
        for ext in report_ext:
            if files.endswith(ext):
                ...
                data['dfile']             = os.path.isdir(report_path+'/'+files[:-len(ext)])
                data['dstatus']           = os.access(report_path+'/'+files[:-len(ext)], os.R_OK)
                if data['dfile']:
                  dfile = get_status(report_path+'/'+files[:-len(ext)])
                  data['acc_permission']  = number_to_character(oct(dfile.st_mode))
                repo.append(data)
...
def read(filename, rformat=None):
    if rformat:
        return report_path+filename+'/sos_reports/sos.{}'.format(rformat.lower())
...
def extract(filename):
    try:
       tmpdir = os.getcwd()
       cmd = "tar -xvf {} > /tmp/null_{}".format(report_path+filename, os.getpid())

       execute = Popen(cmd, shell=True, bufsize=1024,
                stdin=PIPE, stdout=PIPE, stderr=PIPE, close_fds=True, cwd=report_path)
       status = execute.wait()
       if status != 0:
