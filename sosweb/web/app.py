...
def reports(page=1):
    """Show current node reports list."""
...
def html_report(report=None):
    ...
    if html_report:
       return render_template('report_details.html', plugins=html_report, report_name=report)
    else:
       html_report = collect.read(report, 'HTML')
       data = ''
       with open(html_report) as fread:
          return "".join(fread.readlines()).replace('donot.css',
                    '/static/css/donot.css').replace('a href="../', 
...
def report(path=None):
    ...
    try:
      with open(sos) as fread:
	       return "<br>".join(fread.readlines())
    except:
      return "Can not read permission"
